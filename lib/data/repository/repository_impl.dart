...
import 'package:advanced_flutter_arabic/data/mapper/mappar.dart';
  ...
  Future<Either<Failure, String>> forgotPassword(String email) async {
    if (await _networkInfo.isConnected) {
      try {
        ...
        final respons = await _remoteDataSource.forgotPassword(email);
        ...
        if (respons.status == ApiInternalStatus.SUCCESS) {
          ...
          // failure
          // return left
          return Left(Failure(respons.status ?? ResponseCode.DEFAULT,
...
// cache is not existing is not valid

// it's the time to get from API side
      if (await _networkInfo.isConnected) {
        // it's connected to internet , it's safe to call API
